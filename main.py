...
def finetune_model(model, class_num):
    _class_embed = nn.Linear(256, class_num)
    class_embed_layerlist = [_class_embed for i in range(6)]
    model.class_embed = nn.ModuleList(class_embed_layerlist)
    model.transformer.decoder.class_embed = nn.ModuleList(class_embed_layerlist)
    return model 
